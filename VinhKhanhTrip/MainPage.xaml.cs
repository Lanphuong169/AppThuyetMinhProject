...
namespace VinhKhanhTrip
{
    public partial class MainPage : ContentPage
    {
        ...
        private void UpdateUIStrings()
        {
            ...
            if (_closestPoi != null)
            {
                lblTargetName.Text = $"{LanguageManager.Get("Arrive")}: {_closestPoi.Ten}";
            }
            // Đổi tiêu đề TabBar (Cần AppShell lắng nghe)
        }
        ...
        {
            if (_closestPoi != null && _closestPoi != q)
            {
                if (_closestPoi.HighlightCircle != null) _closestPoi.HighlightCircle.StrokeColor = Colors.Transparent;
            }
        ...
        private void OnSuggestionSelected(object sender, SelectionChangedEventArgs e)
        {
            if (e.CurrentSelection.FirstOrDefault() is QuanAn selected)
            {
                ...
                {
                    vitriFake = new Location(selected.Lat - 0.0003, selected.Lng - 0.0003);
                    XuLyDiChuyen();
                }
            }
